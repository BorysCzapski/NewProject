// ============================================================================
// components/matma/exam/ai-feedback-card.tsx
// Own local copy of the shared AiFeedbackCard shape (see build brief — every
// area that shows AI grading builds its own copy instead of importing across
// parallel work areas, since sibling directories may not exist yet at write
// time). Renders a MathAiFeedback: a big "X / Y pkt" headline, one row per
// step_breakdown entry (check/x icon + step name + justification), then the
// improvement_tip in a highlighted callout. Scoped to the exam area only.
// ============================================================================
...
export function AiFeedbackCard({ feedback }: { feedback: MathAiFeedback }) {
  ...
  const scoreClassName =
    ratio >= MIN_MASTERY_THRESHOLD ? "text-accent" : ratio >= 0.5 ? "text-warning" : "text-danger";
